...
*While HVM aims to be the ultimate functional runtime, it, at this point, is to be seen as a prototype. It still doesn't cover all the lambda terms - we plan to do so by adding an optimized version of brackets/croissants (page 40 of [The Optimal Implementation Of Functional Programming Languages](https://www.researchgate.net/publication/235778993_The_optimal_implementation_of_functional_programming_languages)), together with [EAL Inference](https://arxiv.org/pdf/cs/0305011.pdf) to avoid the associated overhead - but we still don't have that yet. As such, HVM won't soundly evaluate certain λ-terms, such as `λx.(x x) λf.λx.(f (f x))` - see HOW.md for more info.*

*Regarding performance, while HVM can be exponentially faster than GHC and other functional runtimes, it was noted that HVM can be quadratic in some programs where Haskell is linear. We're still researching the culprit, and working on solutions to this issue (candidates: "safe pointers", as proposed on the book, and used on [BOHM](https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=351703af066fd2e65ad64c63108672d960b9d8fb); a linearizing pass; or Haskell-like shared references).*

*Moreover, note that HVM is still a prototype with relatively small man-hours spent on it. It doesn't include several low-level optimizations that would make it much faster. As such it won't have great performance in all cases. As an example, Haskell programs that use mutability, state monads, or that compile to tight loops, will be considerably faster than anything HVM can achieve today - but these are optimizations that could be applied to HVM in the future. Currently, in a single core, HVM tends to range from 2x faster to 3x slower than GHC. With enough parallelism, HVM sometimes takes the lead, but not always, and some algorithms are inherently sequential. And, obvisouly, HVM doesn't stand a chance against manually optimized C code with tons of loops and in-place mutations. (Yet!)*

*As for our benchmarks, we believe that, to create faithful and representative benchmarks, we must choose real-world algorithms (like sorting) and make a 1-to-1 translation from Haskell to HVM - after all, that's what a Haskell->HVM compiler would do, roughly. That's why our benchmarks look like almost verbatin conversions from Haskell to HVM syntax. Some of these Haskell algorithms could be improved by, say, using ST monads or mutable arrays, but that'd change the program that runs, and, since HVM doesn't support these (yet), making a 1-to-1 comparison wouldn't be possible.*

*Good news is, all these limitations could be solved given proper resources. There is no foundamental or theoretical barrier. Supporting full lambdas, for example, can be solved via brackets/croissants, and optimized via EAL, as well documented on the literature. As for raw performance, people assume that interaction nets are an overhead, but the opposite is true: it is the absolute lack of overhead. Every primitive is as memory-compact as possible; for example, lambdas are represented with just 2 memory pointers (variable and body), and all computational primitives are lightweight, constant-time rewrite rules. There is no garbage-collection, reference spaghetti or memory thunks. Thinks like loops, mutable arrays, inner mutability (like Lean's FBIP) and so on apply to HVM. A well-optimized HVM could, in theory, reach the same single-threaded performance as Rust, as both share the same common core (an affine λ-calculus), while keeping the high-level feel of a functional language. We're working hard to address all these limitations and turn HVM from a promising tech demo into a production ready runtime that can be targeted by most programming languages.*
