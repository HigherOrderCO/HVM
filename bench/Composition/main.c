          ...
          //GENERATED_REWRITE_RULES_STEP_1_START//
          {
            case _COMP_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == U32 && get_val(ask_arg(mem, term, 0)) == 0u)) {
                ...
                link(mem, app_0 + 0, ask_arg(mem, term, 1));
                link(mem, app_0 + 1, ask_arg(mem, term, 2));
                u64 done = App(app_0);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 3);
                init = 1;
              ...
              if (1) {
                ...
                if (get_tag(ask_arg(mem, term, 0)) == U32 && get_tag(U_32(1)) == U32) {
                  ret_5 = U_32(get_val(ask_arg(mem, term, 0)) - get_val(U_32(1)));
                  inc_cost(mem);
                } else {
                  u64 op2_6 = alloc(mem, 2);
                  link(mem, op2_6 + 0, ask_arg(mem, term, 0));
                  link(mem, op2_6 + 1, U_32(1));
                  ret_5 = Op2(SUB, op2_6);
                }
                u64 lam_7 = alloc(mem, 2);
                u64 app_9 = alloc(mem, 2);
                link(mem, app_9 + 0, dp1_2);
                link(mem, app_9 + 1, Var(lam_7));
                u64 app_8 = alloc(mem, 2);
                link(mem, app_8 + 0, dp0_1);
                link(mem, app_8 + 1, App(app_9));
                link(mem, lam_7 + 1, App(app_8));
                u64 cal_10 = alloc(mem, 3);
                link(mem, cal_10 + 0, ret_5);
                link(mem, cal_10 + 1, Lam(lam_7));
