...
impl Pair {
    ...
    pub fn set_par_flag(&self) -> Self {
        ...
        if p1.get_tag() == REF {
            Pair::new(Port::new(p1.get_tag(), p1.get_val() | 0x10000000), p2)
        } else {
            Pair::new(p1, p2)
        }
    }
...
impl<'a> GNet<'a> {
    ...
    pub fn vars_load(&self, var: usize) -> Port {
        Port(self.vars[var].0.load(Ordering::Relaxed))
    }
    ...
    pub fn vars_exchange(&self, var: usize, val: Port) -> Port {
        Port(self.vars[var].0.swap(val.0, Ordering::Relaxed))
    }
...
impl TMem {
    ...
    pub fn interact_oper(&mut self, net: &GNet, a: Port, b: Port) -> bool {
        ...
        if b1.get_tag() == NUM {
            ...
            net.node_create(
                self.nloc[0],
                Pair::new(Port::new(a.get_tag(), Numb(a.get_val()).flp_flp().0), b2),
            );
