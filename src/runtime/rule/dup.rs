...
pub fn apply(ctx: ReduceCtx) -> bool {
  ...
  if get_tag(arg0) == LAM {
    ...
    atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Var(get_loc(arg0, 0)), Sup(get_ext(ctx.term), par0));
    atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp0(tcol, get_loc(ctx.term, 0)), Lam(lam0));
    atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp1(tcol, get_loc(ctx.term, 0)), Lam(lam1));
  ...
  //           | dup xB yB = b
    ...
    if tcol == get_ext(arg0) {
      inc_cost(ctx.heap, ctx.tid);
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp0(tcol, get_loc(ctx.term, 0)), take_arg(ctx.heap, arg0, 0));
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp1(tcol, get_loc(ctx.term, 0)), take_arg(ctx.heap, arg0, 1));
      free(ctx.heap, ctx.tid, get_loc(ctx.term, 0), 3);
      free(ctx.heap, ctx.tid, get_loc(arg0, 0), 2);
      return true;
      ...
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp0(tcol, get_loc(ctx.term, 0)), Sup(get_ext(arg0), par0));
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp1(tcol, get_loc(ctx.term, 0)), Sup(get_ext(arg0), par1));
      free(ctx.heap, ctx.tid, get_loc(ctx.term, 0), 3);
      return true;
    }
  }
  ...
  // y <- (K a1 b1 c1 ...)
    ...
    let fari = arity_of(&ctx.prog.arit, arg0);
    if fari == 0 {
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp0(tcol, get_loc(ctx.term, 0)), Ctr(fnum, 0));
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp1(tcol, get_loc(ctx.term, 0)), Ctr(fnum, 0));
      ...
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp0(tcol, get_loc(ctx.term, 0)), Ctr(fnum, ctr0));
      atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp1(tcol, get_loc(ctx.term, 0)), Ctr(fnum, ctr1));
      free(ctx.heap, ctx.tid, get_loc(ctx.term, 0), 3);
    }
    return true;
  }
  ...
  // y <- *
    ...
    inc_cost(ctx.heap, ctx.tid);
    atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp0(tcol, get_loc(ctx.term, 0)), Era());
    atomic_subst(ctx.heap, &ctx.prog.arit, ctx.tid, Dp1(tcol, get_loc(ctx.term, 0)), Era());
