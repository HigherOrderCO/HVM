...
fn hvm_seq_visit(ctx: ReduceCtx) -> bool {
  // When visiting, we must reduce the second argument (index 1) if possible.
  // This code was originally auto-generated by the HVM-to-Rust compiler
  // but I have modified it to reduce some redundancies that are necessary
  // when auto-generating code but not when writing it manually.
  
  // First, we must check whether the second argument can be reduced.
  if is_whnf(load_arg(ctx.heap, ctx.term, 1)) {
    // If it is in normal form, it can not be reduced further.
    return false;
  } else {
    // If it can be reduced, then tell the reducer to reduce it afterwards.
    let vbuf = unsafe { ctx.heap.vbuf.get_unchecked(ctx.tid) };
    unsafe { vbuf.get_unchecked(0) }.store(get_loc(ctx.term, 1), Ordering::Relaxed);
    let goup = ctx.redex.insert(ctx.tid, new_redex(*ctx.host, *ctx.cont, 1));
    *ctx.cont = goup;
    *ctx.host = unsafe { vbuf.get_unchecked(0).load(Ordering::Relaxed) };
    return true;
  }
}

fn hvm_seq_apply(ctx: ReduceCtx) -> bool {
  // The following code simply applies the second argument to the first one.
  // The "seq" magic is done in hvm_seq_visit, not here.
  let cont = load_arg(ctx.heap, ctx.term, 0);
