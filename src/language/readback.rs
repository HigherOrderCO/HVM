...
pub fn as_term(heap: &Heap, prog: &Program, host: u64) -> Box<language::syntax::Term> {
  ...
  fn gen_var_names(_heap: &Heap, _prog: &Program, ctx: &mut CtxName, term: Ptr, _depth: u32) {
    ...
    match runtime::get_tag(term) {
      runtime::LAM => {
        ...
        gen_var_names(_heap, _prog, ctx, body, _depth + 1);
      }
      runtime::APP => {
        let lam = runtime::load_arg(ctx.heap, term, 0);
        let arg = runtime::load_arg(ctx.heap, term, 1);
        gen_var_names(_heap, _prog, ctx, lam, _depth + 1);
        gen_var_names(_heap, _prog, ctx, arg, _depth + 1);
      }
      runtime::SUP => {
        let arg0 = runtime::load_arg(ctx.heap, term, 0);
        let arg1 = runtime::load_arg(ctx.heap, term, 1);
        gen_var_names(_heap, _prog, ctx, arg0, _depth + 1);
        gen_var_names(_heap, _prog, ctx, arg1, _depth + 1);
      }
      runtime::DP0 => {
  ...
  fn readback(_heap: &Heap, _prog: &Program, ctx: &mut CtxGo, stacks: &mut Stacks, term: Ptr, _depth: u32) -> Box<language::syntax::Term> {
    match runtime::get_tag(term) {
      runtime::LAM => {
        let body = runtime::load_arg(ctx.heap, term, 1);
        let body = readback(_heap, _prog, ctx, stacks, body, _depth + 1);
      ...
      runtime::APP => {
        ...
        let func = readback(_heap, _prog, ctx, stacks, func, _depth + 1);
        let argm = readback(_heap, _prog, ctx, stacks, argm, _depth + 1);
      ...
      runtime::SUP => {
        ...
        if let Some(val) = stack.last() {
          ...
          let got = readback(_heap, _prog, ctx, stacks, val, _depth + 1);
          stacks.push(col, old);
          got
        } else {
          let val0 = runtime::load_arg(ctx.heap, term, 0);
          let val1 = runtime::load_arg(ctx.heap, term, 1);
          let val0 = readback(_heap, _prog, ctx, stacks, val0, _depth + 1);
          let val1 = readback(_heap, _prog, ctx, stacks, val1, _depth + 1);
      ...
      runtime::DP0 => {
        ...
        let result = readback(_heap, _prog, ctx, stacks, val, _depth + 1);
        stacks.pop(col);
        result
      }
      runtime::DP1 => {
      ...
      runtime::CTR | runtime::FUN => {
        ...
        for i in 0 .. arit {
          let arg = runtime::load_arg(ctx.heap, term, i);
          args.push(readback(_heap, _prog, ctx, stacks, arg, _depth + 1));
        }
      ...
      runtime::ARG => {
        Box::new(language::syntax::Term::Var { name: "<arg>".to_string() })
      }
      runtime::ERA => {
        Box::new(language::syntax::Term::Var { name: "<era>".to_string() })
      }
      _ => {
        Box::new(language::syntax::Term::Var { name: format!("<unknown_tag_{}>", runtime::get_tag(term)) })
      }
    }
  }
  ...
  fn dups(heap: &Heap, prog: &Program, term: Ptr, names: &mut HashMap<u64, String>) -> language::syntax::Term {
    ...
    if lets.is_empty() {
      ...
      for (i, (_key, pos)) in lets.iter().enumerate() {
        ...
        let nam0 = if runtime::load_ptr(heap, *pos) == runtime::Era() { String::from("*") } else { format!("a{}", name) };
...
pub fn as_linear_code(heap: &Heap, prog: &Program, host: u64) -> String {
  format!("{}", as_linear_term(heap, prog, host))
}
